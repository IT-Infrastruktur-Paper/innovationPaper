...
One aspect of all Rust-specific tests is the \texttt{\#[wasm\_bindgen]} attribute before every function which is described by mdn web docks as follows \dq wasm-pack uses wasm-bindgen, another tool, to provide a bridge between the types of JavaScript and Rust. It allows JavaScript to call a Rust API with a string, or a Rust function to catch a JavaScript exception\dq (!Quelle mozilla compiling from rust to webassembly). This attribute is essential for the main.js to access the Rust functions when a test is triggered, therefore it is another important of this chapter. 